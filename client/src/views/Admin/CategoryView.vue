            ...
            <div class="dataTable-wrapper dataTable-loading no-footer sortable searchable fixed-columns">
                <div class="dataTable-top">
                    ...
                    <div class="dataTable-search d-flex">
                        <div class="col-4">
                            <a href="#!" data-bs-toggle="modal" data-bs-target="#addModal" class="btn btn-primary btn-sm h-100">{{ $t('add_new_category') }}</a>
                        </div>
                        <div class="col-8">
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="addModalLabel">{{ $t('add_new_category') }}</h5>
      ...
      <form @submit.prevent="addCategory()">
        <div class="modal-body">
            ...
            <div class="mb-3">
                <label class="small mb-1" for="addMenuIcon">{{ $t('icon') }}</label>
                <input v-model="add.icon" required class="form-control" id="addMenuIcon" type="text">
            </div>
        </div>
    ...
    <div class="modal-content">
      ...
      <form @submit.prevent="saveCategory()">
        <div class="modal-body">
            ...
            <div class="mb-3">
                <label class="small mb-1" for="editMenuIcon">{{ $t('icon') }}</label>
                <input v-model="edit.icon" required class="form-control" id="editMenuIcon" type="text">
            </div>
        </div>
        <div class="modal-footer">
            <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">{{ $t('close') }}</button>
...
export default {
    ...
    methods: {
        ...
        async categoryDelete(index){
            const item = this.items[index]
            if(item.step){
                await adminService.categoryDelete(item)
        ...
        async get(){
            const response = await adminService.categoryList(this.table)
        ...
        async saveCategory(){
            const response = await adminService.categorySave(this.edit)
            if(response.status == 1){
        ...
        async addCategory(){
            const response = await adminService.categoryAdd(this.add)
            if(response.status == 1){
