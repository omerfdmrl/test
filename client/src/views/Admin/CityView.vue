...
<div class="container-xl px-4 mt-4">
    <div class="card mb-4">
        <div class="card-header">{{ $t('cities') }}</div>
        <div class="card-body">
            <div class="dataTable-wrapper dataTable-loading no-footer sortable searchable fixed-columns">
                <div class="dataTable-top">
                    ...
                    <div class="dataTable-search d-flex">
                        <div class="col-4">
                            <a href="#!" data-bs-toggle="modal" data-bs-target="#addModal" class="btn btn-primary btn-sm h-100">{{ $t('add_new_city') }}</a>
                        </div>
                        <div class="col-8">
                ...
                <div class="dataTable-container">
                    <table id="datatablesSimple" class="dataTable-table">
                        <thead>
                            ...
                            <th>
                                <a href="#" class="dataTable-sorter">{{ $t('districts') }}</a>
                            </th>
                            <th>
                        ...
                        <tbody>
                            <tr v-for="(city,index) in items" :key="index">
                                <td>{{ city.title }}</td>
                                <td>{{ city.district.length }}</td>
                                <td>
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="addModalLabel">{{ $t('add_new_city') }}</h5>
      ...
      <form @submit.prevent="addRole()">
        <div class="modal-body">
            ...
            <div class="mb-3 row">
                <div class="col-md-10">
                    <label class="small mb-1" for="addMenuPermission">{{ $t('district') }}</label>
                    <input type="text" v-model="add.val" id="addMenuPermission" class="form-control">
                </div>
                <div class="col-md-2">
                    <label class="small mb-1" for="addMenuPermissionButton">{{ $t('add') }}</label>
                    <button type="button" @click="add.district.push(add.val)" class="btn btn-success form-control" id="addMenuPermissionButton">+</button>
                </div>
            </div>
            <div class="mb-3 row" v-for="(item,index) in add.district" :key="index">
                <div class="col-md-10">
                    <input disabled :value="item" type="text" class="form-control mb-1">
                </div>
                <div class="col-md-2">
                    <button type="button" @click="add.district.splice(index,1)" class="btn btn-warning form-control">X</button>
                </div>
            </div>
        </div>
    ...
    <div class="modal-content">
      ...
      <form @submit.prevent="saveRole()">
        <div class="modal-body">
            ...
            <div class="mb-3 row">
                <div class="col-md-10">
                    <label class="small mb-1" for="editMenuPermission">{{ $t('district') }}</label>
                    <input v-model="edit.val" class="form-control" id="editMenuPermission" type="text">
                </div>
                <div class="col-md-2">
                    <label class="small mb-1" for="editMenuPermissionButton">{{ $t('add') }}</label>
                    <button type="button" @click="edit.district.push(edit.val)" class="btn btn-success form-control" id="editMenuPermissionButton">+</button>
                </div>
            </div>
            <div class="mb-3 row" v-for="(item,index) in edit.district" :key="index">
                <div class="col-md-10">
                    <input disabled :value="item" type="text" class="form-control mb-1">
                </div>
                <div class="col-md-2">
                    <button type="button" @click="edit.district.splice(index,1)" class="btn btn-warning form-control">X</button>
                </div>
            </div>
        </div>
        <div class="modal-footer">
...
export default {
    ...
    methods: {
        ...
        async groupDelete(index){
            const item = this.items[index]
            if(item.step){
                await adminService.groupDelete(item)
        ...
        async get(){
            const response = await adminService.cityList(this.table)
        ...
        async saveRole(){
            const response = await adminService.citySave(this.edit)
            if(response.status == 1){
        ...
        async addRole(){
            const response = await adminService.cityAdd(this.add)
            if(response.status == 1){
