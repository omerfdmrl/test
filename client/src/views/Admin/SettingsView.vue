<template>
    ...
    <div class="container-xl px-4 mt-4">
        ...
        <nav class="nav nav-borders">
            <router-link :to="{name: 'admin-settings', query: { tab: tab.tab }}" v-for="(tab,index) in tabs" :key="index" :class="[currentTab == tab.tab ? 'active' : null,'nav-link ms-0']">{{ tab.name }}</router-link>
        </nav>
        <hr class="mt-0 mb-4">
        <component v-if="Object.keys(settings).length" :key="currentTab" @update="update" :settings="settings" :is="tab"></component>
    </div>
</template>
...
export default {
    data(){
        return {
            settings: {},
            currentTab: this.$route.query.tab && ['general', 'mail', 'seo', 'site', 'api'].includes(this.$route.query.tab) ? this.$route.query.tab : 'general',
            tabs: [
