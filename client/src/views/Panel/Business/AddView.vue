    ...
    <div class="row gx-4">
        <div class="col-lg-8">
            <div class="card mb-4">
                ...
                <div class="card-body"><input required @input="data.slug = urlify(data.title)" v-model="data.title" class="form-control" id="postTitleinput required" type="text" :placeholder="$t('enter_your_title...')"></div>
            </div>
        ...
        <div class="col-lg-4">
            <div class="card mb-4">
                ...
                <div class="card-body"><input required v-model="data.slug" class="form-control" id="postTitleinput required" type="text" :placeholder="$t('enter_your_title...')"></div>
            </div>
            <div class="card mb-4">
                <div class="card-header">{{ $t('image') }}</div>
                <div class="card-body">
                    <input required accept="image/png, image/gif, image/jpeg" @input="handleImage" ref="file" class="form-control" id="postTitleinput required" type="file" multiple>
                    <div class="row mt-2">
            ...
            <div class="card mb-4">
                ...
                <div class="card-body"><Select2 :options="categories" v-model="data.category" class="form-control" id="postCategoryinput required" type="text" :placeholder="$t('enter_your_category...')"></Select2></div>
            </div>
            <div class="card mb-4">
                <div class="card-header">{{ $t('tags') }}</div>
                <div class="card-body"><Select2 :settings="{ multiple: true, tags: true}" v-model="data.tag" class="form-control" id="postTaginput required" type="text" :placeholder="$t('enter_your_tags...')"></Select2></div>
            </div>
            <div class="card mb-4">
                <div class="card-header">{{ $t('location') }}</div>
                <div class="card-body">
                    <Select2 v-model="data.city" :options="cities" @select="handleDistrict" class="form-control" id="postLocationinput required1" type="text" :placeholder="$t('enter_your_city...')">
                    </Select2>
                    <Select2 v-if="data.city" v-model="data.district" :options="districts" class="mt-2 form-control" id="postLocationinput required2" type="text" :placeholder="$t('enter_your_district...')">
                    </Select2>
                    <div class="row mt-2">
                        <div class="col-md-5">
                            <input required v-model="data.latitude" :placeholder="$t('latitude')" type="text" class="form-control">
                        </div>
                        <div class="col-md-5">
                            <input required v-model="data.longitude" :placeholder="$t('longitude')" type="text" class="form-control">
                        </div>
                        <div class="col-md-2">
...
<svg xmlns="http://www.w3.org/2000/svg" height="20" width="20"><path fill="white" d="M10 9.583q.542 0 .906-.364.365-.365.365-.886 0-.541-.365-.906-.364-.365-.906-.365t-.896.365q-.354.365-.354.906 0 .542.365.896.364.354.885.354Zm0 7.209q2.604-2.25 3.99-4.5 1.385-2.25 1.385-3.792 0-2.479-1.542-4.01Q12.292 2.958 10 2.958q-2.271 0-3.812 1.532Q4.646 6.021 4.646 8.5q0 1.542 1.375 3.792t3.979 4.5Zm0 1.166q-3.146-2.833-4.677-5.187Q3.792 10.417 3.792 8.5q0-2.938 1.864-4.667Q7.521 2.104 10 2.104t4.354 1.729q1.875 1.729 1.875 4.667 0 1.917-1.542 4.271-1.541 2.354-4.687 5.187ZM10 8.5Z"/></svg></button>
            ...
            <div class="card mb-4">
                ...
                <div class="card-body">
                    <input required type="text" v-model="data.instagram" :placeholder="$t('instagram')" class="form-control mb-2">
                    <input required type="text" v-model="data.twitter" :placeholder="$t('twitter')" class="form-control mb-2">
                    <input required type="tel" v-model="data.tel_no" :placeholder="$t('tel_no')" class="form-control mb-2">
                    <input required type="mail" v-model="data.mail" :placeholder="$t('mail')" class="form-control mb-2">
                </div>
            </div>
        </div>
    </div>
...
export default {
    ...
    methods: {
        ...
        save(status){
            ...
            this.previews.forEach((file, i) => {
                formData.append('images[' + i + ']', file);
            });
            panelService.businessAdd(formData)
        }
    },
    async created(){
