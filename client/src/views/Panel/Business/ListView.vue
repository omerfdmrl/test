...
<div class="container-xl px-4 mt-4">
    <div class="card mb-4">
        <div class="card-header">{{ $t('business_list') }}</div>
        <div class="card-body">
            <div class="dataTable-wrapper dataTable-loading no-footer sortable searchable fixed-columns">
                <div class="dataTable-top">
                    ...
                    <div class="dataTable-search d-flex">
                        <div class="col-4">
                            <router-link :to="{name: 'panel-business-add'}" class="btn btn-primary btn-sm h-100">{{ $t('add_new_business') }}</router-link>
                        </div>
                        <div class="col-8">
                ...
                <div class="dataTable-container">
                    <table id="datatablesSimple" class="dataTable-table">
                        ...
                        <tbody>
                            <tr v-for="(business,index) in items" :key="index">
                                <td>{{ business.title }}</td>
                                <td>{{ formatDate(business.created_at) }}</td>
                                <td>
                                    <div v-if="business.status == 'published'" class="badge bg-green-soft text-green">{{ $t('published') }}</div>
                                    <div v-if="business.status == 'pending'" class="badge bg-yellow-soft text-yellow">{{ $t('pending') }}</div>
                                    <div v-if="business.status == 'draft'" class="badge bg-gray-200 text-dark">{{ $t('draft') }}</div>
                                </td>
                                <td>
                                    <router-link :to="{name: 'panel-business-edit', params: {slug : business.slug, id: business._id}}"><svg xmlns="http://www.w3.org/2000/svg" height="24" width="24"><path d="M5.4 18.9h1l9.925-9.925-1-1.025L5.4 17.875ZM18.3 8.325 15.975 6l.75-.775q.45-.425 1.087-.425.638 0 1.088.425l.6.6q.25.275.25.662 0 .388-.25.663Zm-.625.65L6.8 19.825H4.475v-2.35L15.325 6.65Zm-1.85-.525-.5-.5 1 1.025Z"/></svg></router-link>
                                    <a @click="businessDelete(index)" href="javascript:;"><svg :fill="business.step ? 'red' : null" xmlns="http://www.w3.org/2000/svg" height="24" width="24"><path d="M7.65 19.85q-.625 0-1.075-.45-.45-.45-.45-1.1V6.05H5.15v-.925h3.875v-.75h5.95v.75h3.9v.925H17.9V18.3q0 .65-.45 1.1-.45.45-1.1.45Zm9.325-13.8H7.05V18.3q0 .275.175.45t.425.175h8.7q.225 0 .425-.187.2-.188.2-.438Zm-7.1 10.9h.925V8.025h-.925Zm3.325 0h.925V8.025H13.2ZM7.05 6.05v12.875-.625Z"/></svg></a>
                                </td>
                            </tr>
                        </tbody>
                    </table>
...
export default {
    ...
    methods: {
        async businessDelete(index){
            const item = this.items[index]
            if(item.step){
                await panelService.businessDelete(item)
        ...
        async get(){
            const response = await panelService.businessList(this.table)
